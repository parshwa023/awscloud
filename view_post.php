...
?>
   ...
   <form action="" method="POST">
      ...
      <textarea name="comment_edit_box" required cols="30" rows="10" placeholder="Please enter your answer"><?= $fetch_edit_comment['comment']; ?></textarea>
   ...
   <div class="box-container">
      ...
      <form class="box" method="post">
         ...
         <div class="icons">
            ...
            <button type="submit" name="like_post"><i class="fas fa-bookmark" style="<?php if($confirm_likes->rowCount() > 0){ echo 'color:var(--red);'; } ?>  "></i><span>(<?= $total_post_likes; ?>)</span></button>
         </div>
      ...
      <?php
         ...
         echo '<p class="empty">No questions found!</p>';
      }
      ?>
   </div>
...
<section class="comments-container">
   ...
   <p class="comment-title">Posted comments</p>
      ...
      <?php
         ...
         if($select_comments->rowCount() > 0){
            while($fetch_comments = $select_comments->fetch(PDO::FETCH_ASSOC)){
               $post_id = $fetch_comments['id'];
               $count_comment_upvote = $conn->prepare("SELECT * FROM `upvotes` WHERE comment_id = ?");
               $count_comment_upvote->execute([$post_id]);
               $total_upvote = $count_comment_upvote->rowCount();
               $confirm_upvote = $conn->prepare("SELECT * FROM  `upvotes` WHERE user_id = ? AND comment_id=? ");
               $confirm_upvote->execute([$user_id,$post_id]);

               $count_comment_downvote = $conn->prepare("SELECT * FROM `downvotes` WHERE comment_id = ?");
               $count_comment_downvote->execute([$post_id]);
               $total_downvote = $count_comment_downvote->rowCount();
               $confirm_downvote = $conn->prepare("SELECT * FROM  `downvotes` WHERE user_id = ? AND comment_id=? ");
               $confirm_downvote->execute([$user_id,$post_id]);
      ...
      <div class="show-comments" style="<?php if($fetch_comments['user_id'] == $user_id){echo 'order:-1;'; } ?>">
         ...
         <?php  if($fetch_comments['user_id'] == $user_id){
            ...
            <div class="vote_numbers">
               <p >Upvotes on your comment: <span><?= $total_upvote; ?></span></p>
               <p >Downvotes on your comment: <span><?= $total_downvote;?></span></p>
            </div>
...
<input type="hidden" name="admin_id" value="<?= $fetch_admin_id['admin_id']; ?>">
      <input type="hidden" name="user_name" value="<?= $fetch_profile['name']; ?>">
<button style="background: none;" name="upvote_comment" class="upvotes-button" type="submit"><i style="<?php if($confirm_upvote->rowCount() > 0){ echo 'color:var(--red);'; } ?>  " class="fas fa-thumbs-up"   ><span> (<?= $total_upvote; ?>)</span></i></button>
<button style="background: none;" name="downvote_comment" class="upvotes-button" type="submit"> <i style="<?php if($confirm_downvote->rowCount() > 0){ echo 'color:var(--red);'; } ?>  " class="fas fa-thumbs-down" ><span> (<?= $total_downvote; ?>)</span></i></button> 
